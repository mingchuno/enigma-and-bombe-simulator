...
export type Solution =
  | { status: 'match'; mapping: number[]; pairs: string[]; unknown: string[]; branches: number }
...
export function buildMenu(ciphertext: string, crib: string, offset: number): MenuEdge[] {
  if (!/^[A-Z]+$/.test(ciphertext) || !/^[A-Z]+$/.test(crib))
    throw new Error('Enter ciphertext and a crib using A–Z.');
  if (!Number.isInteger(offset) || offset < 0 || offset + crib.length > ciphertext.length) {
  ...
  return [...crib].map((letter, index) => {
    ...
    return { a: ALPHABET.indexOf(letter), b: ALPHABET.indexOf(ciphertext[position]), position };
  });
}
...
export function scramblerEdges(
  ...
  return menu.map((edge) => {
    ...
    if (!mapping) {
      mapping = Uint8Array.from({ length: 26 }, (_, input) => enigma.scramble(input));
...
export function solveMenu(edges: ScramblerEdge[], options: SolverOptions): Solution {
  ...
  if (!mapping) return { status: exhausted ? 'unresolved' : 'reject', branches };
  const pairs = mapping.flatMap((partner, letter) =>
    partner > letter ? [ALPHABET[letter] + ALPHABET[partner]] : [],
  );
  const unknown = mapping.flatMap((partner, letter) => (partner === -1 ? [ALPHABET[letter]] : []));
...
export function rotorOrders(): Triple<RotorName>[] {
  return ROTOR_NAMES.flatMap((a) =>
    ROTOR_NAMES.filter((b) => b !== a).flatMap((b) =>
      ROTOR_NAMES.filter((c) => c !== a && c !== b).map((c) => [a, b, c] as Triple<RotorName>),
    ),
  );
}
...
export function* searchBombe(options: SearchOptions): Generator<SearchUpdate> {
  ...
  if (!Number.isInteger(options.maxPairs) || options.maxPairs < 0 || options.maxPairs > 13)
  ...
  if (options.crib.length < 8)
    throw new Error('Use a crib of at least 8 letters; longer cribs reduce ambiguous matches.');
  ...
  for (const rotors of orders) {
    ...
    for (let index = 0; index < 17576; index++) {
      ...
      const result = solveMenu(scramblerEdges(config, menu, cache), { maxPairs: options.maxPairs });
      update.tested++;
      update.current = `${rotors.join('–')} / ${windows}`;
      if (result.status === 'unresolved') update.unresolved++;
      if (result.status === 'match') {
        const plaintext = new Enigma({ ...config, plugs: result.pairs.join(' ') }).process(
          options.ciphertext,
        );
        if (plaintext.slice(options.offset, options.offset + options.crib.length) !== options.crib)
        ...
        if (update.candidates.length >= resultLimit) {
          yield { ...update, candidates: [...update.candidates], reason: 'limit' };
          return;
        }
      }
      if (update.tested % 128 === 0) yield { ...update, candidates: [...update.candidates] };
    }
  }
