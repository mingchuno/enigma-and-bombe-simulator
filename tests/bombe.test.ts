...
test('nonzero crib offset steps from message start', () => {
  const cipher = new Enigma(config).process(plaintext);
  const edges = scramblerEdges(config, buildMenu(cipher, plaintext.slice(7), 7));
  const result = solveMenu(edges, { maxPairs: 3 });
  assert.equal(result.status, 'match');
  const output = new Enigma({ ...config, plugs: result.pairs.join(' ') }).process(plaintext);
...
function involutions(size, maxPairs, partial = Array(size).fill(-1), pairs = 0) {
  const next = partial.indexOf(-1);
  if (next < 0) return [partial];
  const results = [];
  for (let partner = next; partner < size; partner++) {
    if (partial[partner] !== -1 || pairs + Number(partner !== next) > maxPairs) continue;
    const copy = [...partial];
    copy[next] = partner;
    copy[partner] = next;
    results.push(...involutions(size, maxPairs, copy, pairs + Number(partner !== next)));
  }
  return results;
}
...
test('solver agrees with exhaustive toy plugboards including disconnected menus and conflicts', () => {
  ...
  for (let seed = 0; seed < 80; seed++) {
    ...
    for (const maxPairs of [0, 1, 3]) {
      ...
      assert.equal(result.status === 'match', exists, `seed ${seed}, pairs ${maxPairs}`);
    }
  }
});
